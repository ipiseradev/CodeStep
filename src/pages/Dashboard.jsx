import { Link, useLocation } from 'react-router-dom'; // Importamos useLocation para el estado activo
...
const SidebarLink = ({ to, icon: Icon, label }) => {
  ...
  const active = location.pathname === to; // Detecta automáticamente si la ruta es activa
  ...
  return (
    ...
    >
      {active && <div className="absolute left-0 w-1 h-6 bg-primary-500 shadow-[0_0_10px_#8bfe06]" />}
...
export default function Dashboard() {
  const displayModules = (modulesData || []).slice(0, 4).map((m, i) => ({
    id: m.id, // Importante para la navegación
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-primary-500/30 font-sans">
      ...
      <div className="relative z-10 flex min-h-screen">
        {/* Sidebar Corregido: Agregada etiqueta de apertura <aside> */}
        <aside className="hidden lg:flex w-64 border-r border-white/5 bg-black/50 backdrop-blur-xl flex-col">
          ...
          <nav className="flex flex-col gap-1">
            {/* Vinculamos con las rutas reales de tu App.jsx */}
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] uppercase tracking-[0.5em] text-primary-500 font-black">Estadísticas de tu cuenta</span>
              </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="h-1 bg-zinc-900 rounded-full overflow-hidden">
                        <motion.div 
                          ...
                          className="h-full bg-primary-500 shadow-[0_0_10px_#8bfe06]" 
                        />
                      </div>
                    </div>
                    {/* Botón dinámico: lleva a la lección 2 (ejemplo) */}
                    <Link to="/lesson/modulo_2" className="group w-fit flex items-center gap-6 bg-white hover:bg-primary-500 px-8 py-4 text-black transition-all duration-500 hover:shadow-primary-500/20">
              ...
              <motion.div variants={itemVariants} className="border border-white/5 bg-zinc-950/40 p-8 rounded-sm">
                ...
                <div className="h-48 flex items-end gap-3 px-2">
                  {[40, 65, 45, 90, 55, 80, 70].map((h, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-4 group cursor-default h-full justify-end">
                      <div className="relative w-full bg-white/[0.02] rounded-t-sm overflow-hidden flex-1">
                        <motion.div 
                          initial={{ height: 0 }} 
                          animate={{ height: `${h}%` }} 
                          transition={{ delay: 0.5 + (i*0.05), duration: 0.8, ease: "backOut" }}
                          className="absolute bottom-0 w-full bg-zinc-800 group-hover:bg-primary-500 transition-colors duration-300" 
                        />
                      </div>
            ...
            <section className="space-y-8">
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
                {displayModules.map((module) => (
                  <Link
                    key={module.title}
                    to={`/lesson/${module.id}`} // Navegación dinámica para las cards
                    className="group block border border-white/5 bg-zinc-950/40 p-6 rounded-sm hover:border-white/20 transition-all duration-500"
                  >
                    <motion.div variants={itemVariants}>
                      <div className="flex justify-between items-start mb-8">
                        ...
                        <span className="text-[10px] font-black italic text-zinc-400">{module.progress}%</span>
                      </div>
                      <div className="space-y-5">
                        <div className="h-[1px] bg-zinc-900 w-full overflow-hidden">
                          <motion.div 
                            ...
                            className="h-full bg-zinc-600 group-hover:bg-primary-500 transition-all duration-700" 
                          />
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-zinc-600 uppercase font-bold tracking-tighter">{module.lessons} Lecciones</span>
                          <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors">
